...
export default function Carousel({ children, refContainer }) {
  ...
  useLayoutEffect(() => {
    if (refDragHandler.current) {
      const cardElements = refDragHandler.current.getElementsByClassName("card")
  ...
  const updatePosition = useCallback((newIndex) => {
  ...
  }, [cardSize, index, threshold, updatePosition])
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      refForwardDragHandler.removeEventListener("transitionend", funcCheckIndex)
    }
